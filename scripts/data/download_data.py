...
COMPETITION_DIR = RAW_DIR / "give-me-some-credit"
...
def main() -> None:
    ...
    expected_files = [
        COMPETITION_DIR / "Data Dictionary.xls",
        COMPETITION_DIR / "cs-test.csv",
        COMPETITION_DIR / "cs-training.csv",
        COMPETITION_DIR / "sampleEntry.csv",
    ]
    if all(path.exists() for path in expected_files) and not args.force:
        print("Dataset already ready: datasets/raw/give-me-some-credit (4 expected files)")
        return
    ...
    archives = sorted(COMPETITION_DIR.glob("*.zip"))
    for archive in archives:
        with zipfile.ZipFile(archive) as bundle:
            bundle.extractall(COMPETITION_DIR)
    ...
    (COMPETITION_DIR / "source.json").write_text(
        json.dumps(
            {
    ...
    print("Dataset ready: datasets/raw/give-me-some-credit (4 expected files)")
